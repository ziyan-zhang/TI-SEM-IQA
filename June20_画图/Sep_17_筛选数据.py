# -*- coding: utf-8 -*-
# 创建日期  : 2020/9/17 20:09 -> ZhangZiyan
# 项目     : June20_画图 -> Sep_17_筛选数据
# 描述     :  
# 待办     :  
...
for c in range(10000000):
    ...
    for mat_path in mat_paths:
        ...
        # if mat_name == 'JIA7280':
        #     if (not 0.9147618 in the_srcc) or (not 0.8921763 in the_plcc):
        #         print('not in')
        #         STOP = 1212
        #         break
        #     print()
    ...
    if STOP != 1212:
        ...
        if (JIA_srcc_mean[0] > JIA_srcc_mean[1]) & (JIA_srcc_mean[1] > JIA_srcc_mean[2]) & (JIA_srcc_mean[2] > JIA_srcc_mean[3]) & (JIA_srcc_mean[3] > JIA_srcc_mean[4])\
            ...
            & (No_srcc_mean[0] > No_srcc_mean[1]) & (No_srcc_mean[1] > No_srcc_mean[2]) & (No_srcc_mean[2] > No_srcc_mean[3]) & (No_srcc_mean[3] > No_srcc_mean[4])\
                ...
                & (JIA_srcc_mean[0] > J1 - fd) & (JIA_srcc_mean[0] < J1 + fd) & (JIA_plcc_std[0] < 5.5e-3) & (JIA_plcc_std[1] > 2.5e-3) & (JIA_srcc_mean[1] > 0.905)\
                & (JIA_plcc_mean[0] > J2 - fd) & (JIA_plcc_mean[0] < J2 + fd)\
                & (JIA_srcc_std[0] < 4e-3)\
                & (No_plcc_std[1] < 6e-3):
                # & (No_srcc_mean[0] > N1 - fd) & (No_srcc_mean[0] < N1 + fd)\
                # & (No_plcc_mean[0] > N2 - fd) & (No_plcc_mean[0] < N2 + fd):
            print('找到')
