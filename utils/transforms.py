...
class toTensor(object):
    ...
    def __call__(self, pics):
        imgs = [torch.from_numpy(pic[:, :, np.newaxis].transpose((2, 0, 1))) for pic in pics]
        # 三通道
...
class RandomSizedCrop(object):  # 如果这个转换被使用的话, 缩放和改变比例可能会影响质量
    ...
    def __call__(self, imgs):
        # 试验10次, 如果仍旧无法得到满足最后if条件的imgs则采用替代方案
        for attempt in range(10):
            area = imgs[0].size[0] * imgs[0].size[1]
            target_area = random.uniform(self.resize_size[0], self.resize_size[1]) * area  # 面积被缩放成的大小
            aspect_ratio = random.uniform(3. / 4, 4. / 3)  # 缩放比例

            w = int(round(math.sqrt(target_area * aspect_ratio)))
            h = int(round(math.sqrt(target_area / aspect_ratio)))

            if random.random() < 0.5:  # 这句话应该是没有用的, w, h 一大一小在aspect_ratio就已经确定了
                w, h = h, w
